  ...
  return (
    <div dir="rtl" className="min-h-screen bg-background p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <p className="text-muted-foreground mb-4">
        {records.length} צוותים · {pending.length} ממתינים לאישור
      </p>
      {pending.length > 0 && (
        <Button
          size="lg"
          className="mb-4 h-14 w-full md:w-auto"
          onClick={async () => {
        ...
        >
          ✅ אשר את כל הממתינים ({pending.length})
        </Button>
      )}
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-bold">
                    {r.projects?.name} · {r.project_teams?.name}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {r.workers_actual ?? "—"}/{r.workers_expected} עובדים
                    {r.start_time &&
                      ` · התחלה ${new Date(r.start_time).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}`}
                    {r.end_time &&
                      ` · סיום ${new Date(r.end_time).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}`}
