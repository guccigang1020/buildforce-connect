...
export const Route = createFileRoute("/corporations/$id")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      { name: "description", content: loaderData?.corp.description ?? "תאגיד כוח אדם מאומת ב-BuildForce." },
    ],
  }),
  notFoundComponent: () => (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-3xl px-4 py-24 text-center md:px-6">
        ...
        <Button asChild className="mt-6 bg-gradient-primary text-primary-foreground"><Link to="/">חזרה לדף הבית</Link></Button>
      </main>
      <SiteFooter />
    </div>
  ),
              ...
              <div className="flex items-end gap-4">
                ...
                <div className="pb-2">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><Star className="h-4 w-4 fill-primary text-primary" /> {corp.rating} ({corp.reviews} דירוגים)</span>
                    <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> {corp.regions}</span>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
            ...
            {showOfferForm && !submitted && (
              <section className="rounded-2xl border border-primary/40 bg-card p-6 shadow-elegant md:p-8">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowOfferForm(false)} className="text-xs text-muted-foreground hover:text-foreground">סגור</button>
                </div>
                <form
                ...
                >
                  <div className="md:col-span-2">
                    ...
                    <select className="h-12 w-full rounded-md border border-input bg-background px-3 text-sm" required>
                      <option value="">בחר בקשה...</option>
                      {activeRequests.map((r) => (
                        <option key={r.id} value={r.id}>#{r.id} · {r.count} {r.role} · {r.location}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <Label className="mb-2 block">מחיר לשעת עובד (₪)</Label>
                    <Input type="number" min="50" max="500" required className="h-12" placeholder="180" />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Textarea rows={3} maxLength={500} placeholder="פרטים נוספים על הצוות, תנאים מיוחדים..." />
                  </div>
                  <div className="md:col-span-2 flex justify-end gap-2">
                    <Button type="button" variant="ghost" onClick={() => setShowOfferForm(false)}>ביטול</Button>
                    <Button type="submit" className="bg-gradient-primary text-primary-foreground">שלח הצעה</Button>
                  </div>
                </form>
              </section>
            )}
                ...
                {activeRequests.map((r) => (
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="text-xs text-muted-foreground">#{r.id} · {r.postedAt}</div>
                        <div className="mt-1 font-bold">{r.count} {r.role}</div>
                        <div className="mt-1 flex flex-wrap gap-x-3 text-xs text-muted-foreground">
                          <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {r.location}</span>
                          <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" /> {r.startDate}</span>
                          {r.budget && <span className="inline-flex items-center gap-1"><Briefcase className="h-3 w-3" /> {r.budget}</span>}
                        </div>
                      </div>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {[
                  ...
                  <div key={i} className="relative rounded-xl border border-border/60 bg-secondary/40 p-5">
                    ...
                    <div className="flex gap-0.5">{Array.from({ length: t.rating }).map((_, j) => <Star key={j} className="h-3.5 w-3.5 fill-primary text-primary" />)}</div>
                    <p className="mt-3 text-sm text-foreground/90">״{t.quote}״</p>
                    <div className="mt-3 text-xs font-semibold text-muted-foreground">— {t.name}</div>
                  </div>
                ))}
              </div>
            </section>
          ...
          <aside className="space-y-4">
            ...
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6">
              ...
              <h3 className="mt-3 text-sm font-bold">{corp.verified ? "תאגיד מאומת" : "ממתין לאימות"}</h3>
...
function SideRow({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return (
