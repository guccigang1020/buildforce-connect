    ...
    <div dir="rtl" className="min-h-screen bg-background p-4 pb-24">
      ...
      {focusTeamId && (
        <div className="mb-3 text-xs bg-primary/10 text-primary rounded p-2 flex items-center justify-between">
          ...
          <a href="/team-leader" className="underline">
            הצג הכל
          </a>
        </div>
      )}
      {teams.length === 0 && (
        <p className="text-muted-foreground">אין צוותים פעילים. צור קשר עם הקבלן.</p>
      )}
    ...
    <Card className="p-4 space-y-3">
      <div>
        ...
        <div className="text-sm text-muted-foreground">
          {team.projects?.name} · {team.expected_workers} עובדים מתוכננים
        </div>
        {status && (
          <div
            ...
            style={{
              background:
                status === "approved" || status === "auto_approved"
                  ? "#16a34a"
                  : status === "exception" || status === "rejected"
                    ? "#dc2626"
                    : "#ca8a04",
              color: "white",
            }}
          ...
          <details>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {(
                ...
                <Button key={k} variant="outline" onClick={() => exception(k)} className="h-12">
                  {l}
                </Button>
              ))}
            </div>
      ...
      {today?.end_time && (
        <p className="text-sm text-muted-foreground">יום העבודה הסתיים — ממתין לאישור הקבלן.</p>
      )}
      ...
      <p className="text-[11px] text-muted-foreground">
        📸 חובה לצלם את כל הפועלים + עצמך באתר. תמונה מהגלריה לא תתקבל.
      </p>
    </Card>
  );
}
