...
for (let i = 0; i < 10; i++) {
  ...
  const enabled = await page.locator('button:not([disabled])', { hasText: "בחר כזוכה" }).count();
  const total = await page.getByRole("button", { name: /בחר כזוכה/ }).count();
  const winPanel = await page.getByText("הזכייה הושלמה").count();
  console.log(`t+${Math.round((Date.now() - t0) / 1000)}s: enabledAwardBtns=${enabled} totalAwardBtns=${total} winPanel=${winPanel}`);
  if (total === 0 && winPanel > 0) break;
}
await browser.close();
